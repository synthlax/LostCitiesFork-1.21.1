...
public class CompiledPalette {
    ...
    private final Object[] fastPalette = new Object[65536];
    private final Palette.Info[] fastInformation = new Palette.Info[65536];
    ...
    private void buildFastLookup() {
        Arrays.fill(fastPalette, null);
        Arrays.fill(fastInformation, null);
        for (Map.Entry<Character, Object> entry : palette.entrySet()) {
            char key = entry.getKey();
            fastPalette[key] = entry.getValue();
        }
        for (Map.Entry<Character, Palette.Info> entry : information.entrySet()) {
            char key = entry.getKey();
            fastInformation[key] = entry.getValue();
        }
    }
    ...
    public boolean isDefined(Character c) {
        return c != null && fastPalette[c] != null;
    }
    ...
    public Palette.Info getInfo(Character c) {
        return c == null ? null : fastInformation[c];
    }
